...
class Graph_Dataset(InMemoryDataset):
    ...
    def process_set(self, dataset):
        ...
        for target, category in enumerate(categories):
            ...
            paths = glob.glob('{}/*.txt'.format(folder, category))
            for path in paths:
                edge_index, node_features, edge_attr, pseudo, zero_adj = spheres2graph(path, truncated_num, resolution)
                if (zero_adj==False):
                    data = Data(x=node_features, edge_index=edge_index, edge_attr=edge_attr, pseudo=pseudo)
